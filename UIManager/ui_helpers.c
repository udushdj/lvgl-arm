/**
 * @file ui_helpers.c
 * @brief SquareLine Studio 自动生成的辅助函数实现
 *
 * 包含:
 *   - 控件属性统一设置封装 (bar/basic/dropdown/image/label/roller/slider)
 *   - 屏幕切换动画 _ui_screen_change (lv_scr_load_anim封装)
 *   - 屏幕删除 _ui_screen_delete
 *   - 键盘管理 _ui_keyboard_set_target
 *   - 动画回调 (x/y/width/height/opacity/zoom/angle)
 *   - flag/state修改封装 _ui_flag_modify / _ui_state_modify
 *
 * 由 SquareLine Studio 1.4.2 生成，LVGL 8.3.11。
 * 手动修复: _ui_screen_delete 空指针判断 (2026-06-08)
 */
